...
const deleteUser = async (req, res) => {
  try {
    const usersArray=JSON.parse(req.params.id)
    const eliminarUsuarios = await userServices.eliminarById(usersArray);
    if (eliminarUsuarios) {
      res.status(200).json({
        status: 200,
        msj: "Usuarios eliminados correctamente.",
      });
    } else {
      res.status(400).json({
        status: 400,
        msj: "No se han podido eliminar los Usuarios correctamente.",
      });
    }
} catch (error) {
    res.status(400).json({
